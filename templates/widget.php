...
<?php if ( $error ) : ?>
	...
	$pct   = $limit > 0 ? (int) min( 100, round( ( $done / $limit ) * 100 ) ) : 0;
	$step  = (int) ( round( $pct / 5 ) * 5 );
	$plan  = (string) ( $account['plan_name'] ?? '' );
	?>
	<div class="wcd-widget-planrow">
		<?php if ( '' !== $plan ) : ?>
			<span class="ui small text"><strong><?php echo esc_html( $plan ); ?></strong></span>
		<?php endif; ?>
	...
	<table class="wcd-widget-table">
		<tr><td><?php esc_html_e( 'Checks remaining', 'webchangedetector-for-mainwp' ); ?></td><td><?php echo esc_html( number_format_i18n( $left ) ); ?></td></tr>
	...
	<div class="wcd-widget-actions">
		<a href="<?php echo esc_url( $visual_checks_url ); ?>" class="ui mini basic button"><?php esc_html_e( 'Visual Checks', 'webchangedetector-for-mainwp' ); ?></a>
