	...
	<div class="ui grid">
		<div class="equal width row ui mini form">
			...
			<div class="right aligned middle aligned column wcd-runs-filters">
				<div id="wcd-runs-period" class="ui selection dropdown not-auto-init">
				...
				<span class="wcd-runs-daterange" hidden>
					<input type="date" id="wcd-runs-from" value="<?php echo esc_attr( $wcd_mainwp_from_initial ); ?>" aria-label="<?php esc_attr_e( 'From', 'webchangedetector-for-mainwp' ); ?>">
					<input type="date" id="wcd-runs-to" value="<?php echo esc_attr( $wcd_mainwp_to_initial ); ?>" aria-label="<?php esc_attr_e( 'To', 'webchangedetector-for-mainwp' ); ?>">
				</span>
				<div id="wcd-runs-status" class="ui search selection multiple dropdown">
					<input type="hidden" value="">
					<i class="dropdown icon"></i>
					<input class="search" autocomplete="off" tabindex="0">
					<div class="default text"><?php esc_html_e( 'All statuses', 'webchangedetector-for-mainwp' ); ?></div>
					<div class="menu">
						<?php foreach ( $wcd_mainwp_status_options as $wcd_mainwp_status_key => $wcd_mainwp_status_label ) : ?>
							<div class="item" data-value="<?php echo esc_attr( $wcd_mainwp_status_key ); ?>"><?php echo esc_html( $wcd_mainwp_status_label ); ?></div>
						<?php endforeach; ?>
					</div>
				</div>
				<div id="wcd-runs-website" class="ui search selection multiple dropdown">
					<input type="hidden" value="">
					<i class="dropdown icon"></i>
					<input class="search" autocomplete="off" tabindex="0">
					<div class="default text"><?php esc_html_e( 'All websites', 'webchangedetector-for-mainwp' ); ?></div>
					<div class="menu">
						<?php foreach ( $wcd_mainwp_website_options as $wcd_mainwp_site ) : ?>
							<div class="item" data-value="<?php echo esc_attr( (string) $wcd_mainwp_site['site_id'] ); ?>"><?php echo esc_html( $wcd_mainwp_site['name'] ); ?></div>
						<?php endforeach; ?>
					</div>
				</div>
